...
public class OPC
{
  ...
  OPC(String host, int port, int wid, int hgt)
  {
    this.host = host;
    this.port = port;
    this.enableShowLocations = true;
    this.captureWidth = wid;
    this.captureHeight = hgt;
  }

  // Set the location of a single LED
  void led(int index, int x, int y, int group)  
  {
    ...
    pixelGroups[index] = group;
  }
  
  // Set the location of several LEDs arranged in a strip.
  // Angle is in radians, measured clockwise from +X.
  // (x,y) is the center of the strip.
  void ledStrip(int index, int count, float x, float y, float spacing, float angle, boolean reversed, int group)
  {
    float s = sin(angle);
    float c = cos(angle);
  ...
  void ledGrid(int index, int stripLength, int numStrips, float x, float y,
               float ledSpacing, float stripSpacing, float angle, boolean zigzag, int group)
  {
    ...
    for (int i = 0; i < numStrips; i++) {
      ledStrip(index + stripLength * i, stripLength,
        ...
        angle, zigzag && (i % 2) == 1, group);
    }
  }

  // Set the location of 64 LEDs arranged in a uniform 8x8 grid.
  // (x,y) is the center of the grid.
  void ledGrid8x8(int index, float x, float y, float spacing, float angle, boolean zigzag, int group)
  {
    ledGrid(index, 8, 8, x, y, spacing, spacing, angle, zigzag, group);
  }
  ...
  void capture(PImage pg, int group)
  {
    ...
    for (int i = 0; i < numPixels; i++) {
      ...
      if (pixelGroups[i] == group && pixelLocation >= 0 && pixelLocation < pg.pixels.length) {
        ...
        if (enableShowLocations)
          pg.pixels[pixelLocation] ^= 0xFFFFFF;
      }
      ledAddress += 3;
    }
